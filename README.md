...
## 2\. Assignment 2 Clarifications (Updated)

To provide flexibility and support multiple valid design approaches, the following clarifications apply:

### 2.1 Command-Line Arguments

- **Sender** has an optional `[window_size]` argument:
  - Omit `window_size` → Stop-and-Wait
  - Provide `window_size` → Go-Back-N

- **Receiver** does **not** require a `window_size` argument by default
  - However, you **may optionally** introduce a `[window_size]` argument for the Receiver with the same logic as the Sender if desired

### 2.2 Sequence Numbers

- Sequence numbers range from **0 to 127** (modulo 128) for both protocols

- **StopAndWait Note:** Although the course teaches that Stop-and-Wait only needs sequence numbers 0 and 1, you **may use 0-127** for consistency
  - Alternatively, you are allowed to use **only 0 and 1** if desired, making your implementation match the lecture notes exactly

### 2.3 Universal Receiver Design

The current requirements allow for a "**universal**" receiver that properly handles **both** GoBackN and StopAndWait senders without modification. If implementing a universal receiver is challenging, you may simplify by:

- Adding an optional `[window_size]` argument to the Receiver (same logic as Sender)
- Using only two sequence numbers (0 and 1) for StopAndWait
- Implementing separate protocol logic in the Receiver based on configuration

* * *

## 3\. Technical Contract: DS-FTP Packet Format

All communication occurs over **UDP**, but reliability is provided **entirely by your protocol**.

### 3.1 Packet Size Requirement
...
## 8\. Timeout and Critical Failure
...
## 10\. Chaos Factor: Unreliability Simulation

You **must use `ChaosEngine.java`** exactly as provided.

### 10.1 Lost ACKs (Receiver Side)
...
### 10.2 Out-of-Order Delivery (Sender Side – GBN Only)
...
## 11\. Command-Line Interface (Exact Syntax Required)
